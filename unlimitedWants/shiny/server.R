...
SOURCE.DIR <- "/home/stephen/Dropbox/research/poorhouse/poorHouse/unlimitedWants/"
...
SIM.CLASS.NAME <- "edu.umw.poorhouse.Model"
...
CLASSPATH <- paste(
    paste(SOURCE.DIR,LIBS,sep="/",collapse=":"),
    CLASSES.DIR,sep=":")
...
shinyServer(function(input,output,session) {
    ...
    # COMM.STATS.FILE.
    comm.stats <- function() {
        if (!file.exists(sub("SIMTAG",simtag,COMM.STATS.FILE))) {
            return(data.frame())
        }
        tryCatch({
            # Change the colClasses argument here, if desired, to control the
            # classes used for each of the data columns.
            read.csv(sub("SIMTAG",simtag,COMM.STATS.FILE),header=TRUE,
                colClasses=c("integer","numeric","numeric","numeric","factor"),
                stringsAsFactors=FALSE)
        },error = function(e) return(data.frame())
        )
    }
    ...
    observe({
        ...
        if (nrow(sim.stats.df) > 0) {
            progress$set("Running simulation...",
                detail=paste(max(sim.stats.df$year),"of",maxTime,
                    "years"),
                value=1+max(sim.stats.df$year))
            if (max(sim.stats.df$year) == maxTime) {
...
#            }
    ...
    output$consumptionPlot <- renderPlot({
        ...
        sim.stats.df <- sim.stats()
        if (nrow(sim.stats.df) > 0) {
            the.plot <- ggplot(sim.stats.df,aes(x=year,y=totalConsumed)) + 
    ...
    output$commodityPricesPlot <- renderPlot({
        if (input$runsim < 1) return(NULL)
        comm.stats.df <- comm.stats()
        if (nrow(comm.stats.df) > 0) {
            the.plot <- ggplot(comm.stats.df,
                    aes(x=year,y=avg.price,col=commodity)) + 
                geom_line() +
                scale_x_continuous(limits=c(1,isolate(input$maxTime)),
                                    breaks=1:isolate(input$maxTime)) +
                expand_limits(y=0) +
                labs(title="Average commodity price",x="Year") +
